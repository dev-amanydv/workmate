      ...
      <div className="flex-1 overflow-y-auto py-1">
        {conversations.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 px-6 text-center py-16">
            <div className="h-12 w-12 rounded-lg border border-[#EDECE8] bg-[#F5F4F0] flex items-center justify-center text-[#184A45]">
            ...
            <p className="text-sm font-semibold text-[#17191A]">No conversations yet</p>
            <p className="text-xs text-[#6C6F71]">Start a conversation from someone&apos;s profile</p>
          </div>
        ) : (
          conversations.map((conv) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[#F5F4F0] cursor-pointer ${
                  isActive ? "bg-[#F5F4F0]" : ""
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline justify-between gap-1">
                    <span
                      className={`text-sm truncate ${isActive ? "font-semibold text-[#000000]" : "font-medium text-[#17191A]"}`}
                    >
                      {other?.name ?? "Unknown"}
                    </span>
                    {lastMsg && (
                      <span className="text-[11px] text-[#8A8D90] flex-shrink-0 tabular-nums">
                        {timeAgo(lastMsg.createdAt)}
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-[#6C6F71] truncate mt-0.5">
                    {lastMsg
